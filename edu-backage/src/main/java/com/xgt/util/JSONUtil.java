...
public class JSONUtil {
	...
	public static Object jsonToObject(String json, Class className) {
		net.sf.json.JSONObject o = net.sf.json.JSONObject.fromObject(json);
		return net.sf.json.JSONObject.toBean(o, className);
	}
	...
	private static String process(Object object,
		...
		if (keySet.contains(METHOD_PROPERTY_FILTER)) {
			// 过滤属性
			final String[] keys = (String[]) methodsParam.get(METHOD_PROPERTY_FILTER);
			// 使用binarySearch一定要先排序
			Arrays.sort(keys);
